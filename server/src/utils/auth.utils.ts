...
export const getPasswordHash = async (plainTextPassword: string): Promise<string> => {
    const saltRounds = 10;
...
export const validateToken = async (token: string): Promise<{ id: string, email: string } | null> => {
    try {
